...
struct Calculator {
    ...
    mutating func setDecimal() {
        guard !containsDecimal else {return}
        self.carryingDecimal = true
    }
    
    mutating func evaluate() {
        guard let number = newNumber, let expressionToEvaliate = expression else { return }
        result = expressionToEvaliate.evaluate(with: number)
        expression = nil
        newNumber = nil
    }
    ...
    private func getNumberString(forNumber number: Decimal?, withCommas: Bool = false) -> String {
        ...
        if carryingZeroCount > 0 {
            numberString += String(repeating: "0", count: carryingZeroCount)
        }
        return numberString
    }
